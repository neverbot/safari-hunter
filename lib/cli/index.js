...
program
  .version('1.0.0', '-v, --version', 'output the current safari-hunterversion')
  .option(
    '-b, --bookid <bookid>',
    'the book id of the safari-hunter ePub to be generated'
  )
  .option(
    '-u, --username <username>',
    'username of the safari-hunter user - must have a **paid/trial membership**, otherwise will not be able to access the books'
  )
  .option('-p, --password <password>', 'password of the safari-hunter user')
  .option(
    '-o, --output <output>',
...
if (fs.existsSync(__dirname + '/.credentials')) {
  var credentials = JSON.parse(
    fs.readFileSync(__dirname + '/.credentials', 'utf-8')
  );
...
fs.writeFile(__dirname + '/.credentials', json)
  .then(() => {
