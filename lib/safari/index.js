 ...
 */
export const Safari = function Safari() {
...
Safari.prototype.fetchStylesheet = function fetchStylesheet(id) {
  ...
  if (!this.books[id] || !this.books[id].chapters) {
    return Promise.reject(
      "the chapters haven't beeen fetched yet. please call fetchChapters before."
    );
  }
  ...
  if (cssSet.size > 1) {
    log(
      `an error occurred while fetching stylesheets. there are ${cssSet.size} different stylesheets. only supporting one. the layout may be different from the one online. sorry for that.`
    );
    debug(
      `an error occurred while fetching stylesheets. there are ${cssSet.size} different stylesheets. only supporting one.`
    );
...
Safari.prototype.authorizeUser = function authorizeUser(username, password) {
  ...
  if (!username || !password)
    return Promise.reject('username or password was not specified');
  ...
  return request(options)
    .then((body) => {
      ...
      if (!accessToken) {
        debug(
          'the access_token is not present in the server response, even though it returned an 200 OK'
        );
        return Promise.reject(
          'The access_token is not present in the server response. Please contact the developer to fix this problem.'
        );
      }
    ...
    .catch((err) => {
      // an error occured
      debug(
        `an error occured while trying to fetch authorization token (error: ${err})`
      );
...
Safari.prototype.fetchResource = function fetchResource(url, options) {
  ...
  if (!url || !this.accessToken)
    return Promise.reject(
      'url was not specified or user has not been authorized yet'
    );
  ...
  return request(settings)
    ...
    .catch((err) => {
      // an error occurred
      debug(
        `there was an unexpected error fetching the resource (err: ${err})`
      );
      return Promise.reject(err);
    });
};
...
Safari.prototype.fetchChapters = function fetchChapters(id) {
  ...
  if (!id) return Promise.reject('id was not specified');
  if (!this.books[id] || !this.books[id].meta || !this.books[id].toc)
    return Promise.reject('the book meta has not been fetched yet');
  // fetch chapter meta data
  let promises = this.books[id].meta.chapters.map((chapterUrl) => {
    return this.fetchResource(chapterUrl, { uri: chapterUrl }).then(
      (chapterMeta) => {
        if (!chapterMeta.content)
          return Promise.reject(
            "the books 'content' key is missing from the response."
          );
        // chapter meta fetched successfully
        // fetch chapter content for now
        return this.fetchResource(chapterMeta.content, {
          uri: chapterMeta.content,
          json: false
        }).then((chapterContent) => {
...
Safari.prototype.getBookById = function getBookById(id) {
  ...
  if (!id) return Promise.reject('id was not specified');
  if (!this.books[id])
    return Promise.reject('the book you requested was not fetched yet');
  let book = this.books[id];
  if (!book.meta || !book.chapters)
    return Promise.reject(
      'the book you requested is missing some required information'
    );
  let jsonBook = {
 ...
 */
Safari.prototype.fetchBookById = function fetchBookById(
  id,
  username,
  password
) {
